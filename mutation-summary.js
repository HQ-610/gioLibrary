...
var MutationObserverCtor = window.MutationObserver || window.WebKitMutationObserver || window.MozMutationObserver
        ...
        for (var i = 0; i < this.mutations.length; i++) {
            ...
            for (var j = 0; j < mutation.removedNodes.length; j++) {
                ...
                if (change.added.has(node)) {
                    change.added.remove(node);
                }
                else {
                    change.removed.set(node, true);
                    change.maybeMoved.remove(node);
                }
                oldPrevious = node;
            }
...
var Selector = (function () {
    ...
    Selector.prototype.isMatching = function (el) {
      var matchesSelector = el[Selector.matchesSelector];
      if (!matchesSelector) {
        var node = el, parentElement = node.parentNode || node.document, i = -1;
        if (parentElement === null || typeof(parentElement) === "undefined") {
          return false;
        }
        var nodes = parentElement.querySelectorAll(selector);
    ...
    MutationSummary.validateOptions = function (options) {
        ...
        for (var i = 0; i < options.queries.length; i++) {
            ...
            if ('element' in request) {
                ...
                // 上方注释掉的代码是原始实现，Object.keys 会取到原型链中的属性。应该使用 hasOwnProperty 过滤掉。
                // 不要使用 Array.filter ，此方法不支持 IE9.
                var requestOptionCount = 0;
                var requestKeys = Object.keys(request);
                requestKeys.forEach(function (k) {
